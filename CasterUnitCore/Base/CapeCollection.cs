        ...
        CapeOpenBaseObject, ICapeCollection, ICollection<CapeCollectionPair>
    {
        ...
        object ICapeCollection.Item(object id)
        {
            if (id is string)
                ...
                throw new ECapeInvalidArgumentException("index can only be int or string", 0);
        }
        ...
        public void Add(string key, ICapeIdentification item)
        {
            if (_items.ContainsValue(item))
                throw new ECapeBadArgumentException("This Collection already has the same item.", 0);
            if (_items.ContainsKey(key))
                throw new ECapeBadArgumentException("This collection already has an item with the same key.", 0);
            _keys.Add(key);
            _items.Add(key, item);
        }
